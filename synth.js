...
class Synth {
    ...
    play(frequency, masterVolume = 0.5, duration = 1, outputNode = null, velocity = 1.0) {
        const audioContext = Synth.audioContext;
        frequency = isFinite(frequency) ? frequency : 440; // TODO: decide if this is needed
        ...
        mainGainNode.gain.linearRampToValueAtTime(
            isFinite(masterVolume) ? masterVolume : 0.5,
            attackEndTime
        ); // Attack
        mainGainNode.gain.linearRampToValueAtTime(
            isFinite(masterVolume * this.amplitudeEnvelope.sustain) ? masterVolume * this.amplitudeEnvelope.sustain : 0.35,
            decayEndTime
        ); // Decay
        ...
        this.oscillators.forEach((oscConfig) => {
            ...
            if (oscConfig.waveform === WaveformType.NOISE) {
                // TODO: implement noise oscillator
            } else {
